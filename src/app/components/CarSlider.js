"use client";
import React from "react";

import "swiper/css";
import Image from "next/image";

import { FaStar, FaStarHalfAlt, FaRegStar } from "react-icons/fa";

import { motion } from "framer-motion";

import { fadeIn } from "/variants";

import { Swiper, SwiperSlide } from "swiper/react";

const cars = [
  {
    type: "Hatchback",
    name: "Ford Focus",
    price: 29,
    stars: 3.5,
    image: "https://quocbao238.github.io/Car-Shop/images/carSlider/car01.svg",
    info: [
      {
        icon: "https://quocbao238.github.io/Car-Shop/icons/carSlider/gearshift.svg",
        text: "Manual",
      },

      {
        icon: "https://quocbao238.github.io/Car-Shop/icons/carSlider/seat.svg",
        text: "5 Seats",
      },
      {
        icon: "https://quocbao238.github.io/Car-Shop/icons/carSlider/gas.svg",
        text: "Gas",
      },
      {
        icon: "https://quocbao238.github.io/Car-Shop/icons/carSlider/engine.svg",
        text: "1600 HP",
      },
      {
        icon: "https://quocbao238.github.io/Car-Shop/icons/carSlider/wheel.svg",
        text: "Front",
      },
    ],
  }, //
  {
    type: "Sedan",
    name: "Toyota Corolla",
    price: 25,
    stars: 5,
    image: "https://quocbao238.github.io/Car-Shop/images/carSlider/car02.svg",
    info: [
      {
        icon: "https://quocbao238.github.io/Car-Shop/icons/carSlider/gearshift.svg",
        text: "Manual",
      },

      {
        icon: "https://quocbao238.github.io/Car-Shop/icons/carSlider/seat.svg",
        text: "5 Seats",
      },
      {
        icon: "https://quocbao238.github.io/Car-Shop/icons/carSlider/gas.svg",
        text: "Gas",
      },
      {
        icon: "https://quocbao238.github.io/Car-Shop/icons/carSlider/engine.svg",
        text: "1600 HP",
      },
      {
        icon: "https://quocbao238.github.io/Car-Shop/icons/carSlider/wheel.svg",
        text: "Front",
      },
    ],
  }, //
  {
    type: "SUV",
    name: "Honda CR-V",
    price: 35,
    stars: 4.7,
    image: "https://quocbao238.github.io/Car-Shop/images/carSlider/car03.svg",
    info: [
      {
        icon: "https://quocbao238.github.io/Car-Shop/icons/carSlider/gearshift.svg",
        text: "Automatic",
      },

      {
        icon: "https://quocbao238.github.io/Car-Shop/icons/carSlider/seat.svg",
        text: "5 Seats",
      },
      {
        icon: "https://quocbao238.github.io/Car-Shop/icons/carSlider/gas.svg",
        text: "Gas",
      },
      {
        icon: "https://quocbao238.github.io/Car-Shop/icons/carSlider/engine.svg",
        text: "1600 HP",
      },
      {
        icon: "https://quocbao238.github.io/Car-Shop/icons/carSlider/wheel.svg",
        text: "Front",
      },
    ],
  }, //
  {
    type: "Convertible",
    name: "Mazda MX-5",
    price: 32,
    stars: 4.2,
    image: "https://quocbao238.github.io/Car-Shop/images/carSlider/car02.svg",
    info: [
      {
        icon: "https://quocbao238.github.io/Car-Shop/icons/carSlider/gearshift.svg",
        text: "Manual",
      },

      {
        icon: "https://quocbao238.github.io/Car-Shop/icons/carSlider/seat.svg",
        text: "5 Seats",
      },
      {
        icon: "https://quocbao238.github.io/Car-Shop/icons/carSlider/gas.svg",
        text: "Gas",
      },
      {
        icon: "https://quocbao238.github.io/Car-Shop/icons/carSlider/engine.svg",
        text: "1600 HP",
      },
      {
        icon: "https://quocbao238.github.io/Car-Shop/icons/carSlider/wheel.svg",
        text: "Front",
      },
    ],
  },
];

const CarSlider = () => {
  return (
    <motion.div
      className="container mx-auto"
      variants={fadeIn("up", 0.2)}
      initial="hidden"
      whileInView="show"
      viewport={{
        once: true,
        amount: 0.2,
      }}
    >
      <Swiper
        breakpoints={{
          320: { slidesPerView: 1, spaceBetween: 15 },
          640: { slidesPerView: 2, spaceBetween: 32 },
          1260: { slidesPerView: 3, spaceBetween: 32 },
        }}
      >
        {cars.map((car, index) => {
          return (
            <SwiperSlide key={index}>
              <div className="max-w-[385px] mx-auto sm:mx-0">
                <Image width={380} height={284} src={car.image} alt="" />
                <div className="flex justify-between">
                  <div>
                    <div className="text-[13px] text-secondary uppercase">
                      {car.type}
                    </div>
                    <h3 className="text-lg uppercase font-bold">{car.name}</h3>
                    <h3 className="mb-10 text-accent font-semibold uppercase">
                      {car.price}/day
                    </h3>
                  </div>
                  <div className="flex gap-x-2 text-accent h-max">
                    <FaStar />
                    <FaStar />
                    <FaStar />
                    <FaStar />
                    <FaStar />
                  </div>
                </div>
                <div className="flex gap-x-3 xl:gap-x-4 mb-10 justify-between min-w-full">
                  {car.info.map((item, index) => {
                    return (
                      <div key={index}>
                        <div className="bg-primary w-12 h-12 rounded-full flex justify-center items-center mb-2">
                          <Image
                            width={24}
                            height={24}
                            src={item.icon}
                            alt=""
                          />
                        </div>
                        <div className="text-[12px] uppercase text-center">
                          {item.text}
                        </div>
                      </div>
                    );
                  })}
                </div>
                <button className="btn btn-accent btn-lg">See details</button>
              </div>
            </SwiperSlide>
          );
        })}
      </Swiper>
    </motion.div>
  );
};

export default CarSlider;
